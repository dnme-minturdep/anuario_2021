...
data_tabla_subregion <- llegadas_subregion_long %>%  
  ...
  filter(!is.na(`2000`) & subregion != "Mundo") %>%
    select(subregion,`2000`,`2005`,`2010`,`2015`,`2020`) %>% mutate_at(vars(-subregion),as.numeric))
...
tabla_omt_subregion <- data_tabla_subregion %>% 
  select(region,subregion,`2000`,`2005`,`2010`,`2015`,`2020`,llegadas,var_i_a,var_2019,part) %>% 
  group_by(region) %>% 
  gt() %>% 
...
data_tabla_paises <- llegadas_paises %>% 
  ...
  ungroup() %>% 
  filter(anio %in% c(2000,2005,2010,2015,2020,anio_actual)) %>% 
  select(nombre,anio,valor,var_i_a,var_2019) %>% 
  pivot_wider(names_from = anio,values_from = c(valor,var_i_a,var_2019))
```

Las llegadas de turistas internacionales en **`r data_tabla_paises %>% arrange(-valor_2021) %>% slice(1) %>% pull(nombre)`** `r  ifelse(data_tabla_paises %>% arrange(-valor_2021) %>% slice(1) %>% pull(var_i_a_2021) >0,"crecieron",cayeron)` `r round(data_tabla_paises %>% arrange(-valor_2021) %>% slice(1) %>% pull(var_i_a_2021) %>% abs() *100,1 )`%. En comparación con 2019, la variación fue de `r round(data_tabla_paises %>% arrange(-valor_2021) %>% slice(1) %>% pull(var_2019_2021)  *100,1 )`%. El siguiente país en magnitud fue **`r data_tabla_paises %>% arrange(-valor_2021) %>% slice(2) %>% pull(nombre)`**, que `r  ifelse(data_tabla_paises %>% arrange(-valor_2021) %>% slice(2) %>% pull(var_i_a_2021) >0,"aumentó","disminuyó")` sus llegadas un `r round(data_tabla_paises %>% arrange(-valor_2021) %>% slice(2) %>% pull(var_i_a_2021) %>% abs() *100,1 )`% con respecto a `r as.character(anio_actual-1)` (`r round(data_tabla_paises %>% arrange(-valor_2021) %>% slice(2) %>% pull(var_2019_2021)  *100,1 )`% vs. 2019). Por su parte, las llegadas a **`r data_tabla_paises %>% arrange(-valor_2021) %>% slice(3) %>% pull(nombre)`** `r  ifelse(data_tabla_paises %>% arrange(-valor_2021) %>% slice(3) %>% pull(var_i_a_2021) >0,"se elevaron","se contrajeron")` un `r round(data_tabla_paises %>% arrange(-valor_2021) %>% slice(3) %>% pull(var_i_a_2021) %>% abs() *100,1 )`% (`r round(data_tabla_paises %>% arrange(-valor_2021) %>% slice(3) %>% pull(var_2019_2021)  *100,1 )`% vs. 2019).


```{r tabla-omt-paises, fig.cap=""}
tabla_llegadas_paises <- data_tabla_paises %>%
  select(nombre,valor_2000,valor_2005,valor_2010,valor_2015,valor_2020,valor_2021,var_i_a_2021,var_2019_2021) %>% 
  ...
  tab_spanner(columns = c(valor_2021,var_i_a_2021,var_2019_2021),
               label = md(glue("**{anio_actual}**"))) %>% 
  cols_label(nombre = md(""),
             ...
             valor_2015 = md("2015"),
             valor_2020 = md("2020"),
             valor_2021 = md("Llegadas"),
             var_i_a_2021 = md("Var. i.a. (%)"),
             var_2019_2021 = md("Var. vs. 2019 (%)")) %>%
  tab_style(
    style = cell_text(weight =  "bold"),
    locations = cells_row_groups()
  ) %>% 
...
data_graf_llegadas_paises <- 
 ...
 filter(nombre %in% ( data_tabla_paises %>% arrange(-valor_2021) %>% slice(1:10) %>% pull(nombre))) 
```
...
tabla_ingresos_omt_subregion <- data_tabla_ingresos_subregion %>% 
  select(region,subregion,`2000`,`2005`,`2010`,`2015`,`2020`,ingresos,var_i_a,var_2019,part) %>% 
  group_by(region) %>% 
  gt() %>% 
   gt_theme_dnmye()%>% 
  fmt_percent(columns = c(part),decimals = 1, sep_mark = ".",dec_mark = ",") %>% 
...
data_graf_ingresos_paises <-   ingresos_paises %>% 
 ...
 filter(nombre %in% (data_tabla_ingresos_paises %>% arrange(-valor_2021) %>% slice(1:10) %>% pull(nombre))) 
...
data_graf_egresos_paises <-  egresos_paises %>% 
    mutate(valor = as.numeric(valor),
         anio = as.numeric(anio)) %>% 
  filter(indicador == "Travel" & anio > 2004) %>%
 filter(nombre %in% (data_tabla_egresos_paises %>% arrange(-valor_2021) %>% slice(1:10) %>% pull(nombre))) 
...
En América, el país que recibió más turistas en `r as.character(anio_actual)` fue **`r data_tabla_america %>% arrange(-llegadas_2021) %>% slice(1) %>% pull(pais)`**, con `r data_tabla_america %>% arrange(-llegadas_2021) %>% slice(1) %>% pull(llegadas_2021) %>% round(1)` millones; en segundo lugar se ubicó **`r data_tabla_america %>% arrange(-llegadas_2021) %>% slice(2) %>% pull(pais)`**, con `r data_tabla_america %>% arrange(-llegadas_2021) %>% slice(2) %>% pull(llegadas_2021) %>% round(1)` millones; detrás de él estuvo **`r data_tabla_america %>% arrange(-llegadas_2021) %>% slice(3) %>% pull(pais)`**, con `r data_tabla_america %>% arrange(-llegadas_2021) %>% slice(3) %>% pull(llegadas_2021) %>% round(1)` millones; el cuarto lugar lo ocupó **`r data_tabla_america %>% arrange(-llegadas_2021) %>% slice(4) %>% pull(pais)`**, con `r data_tabla_america %>% arrange(-llegadas_2021) %>% slice(4) %>% pull(llegadas_2021) %>% round(1)` millones; la quinta posición fue para **`r data_tabla_america %>% arrange(-llegadas_2021) %>% slice(5) %>% pull(pais)`**, con `r data_tabla_america %>% arrange(-llegadas_2021) %>% slice(5) %>% pull(llegadas_2021) %>% round(1)` millones de llegadas en `r as.character(anio_actual)`

```{r  tabla-omt-america}
tabla_llegadas_america <- data_tabla_america %>%
  select(pais,llegadas_2005,llegadas_2010,llegadas_2015,llegadas_2020,llegadas_2021,var_i_a_2021,var_2019_2021) %>% 
  mutate_at(vars(-pais),as.numeric) %>% 
  arrange(-llegadas_2021) %>% 
  slice(1:20) %>% 
  gt() %>% 
  ...
  tab_spanner(columns = c(llegadas_2021,var_i_a_2021,var_2019_2021),
               label = md(glue("**{anio_actual}**"))) %>% 
  cols_label(pais = md(""),
             llegadas_2005 = md("2005"),
             llegadas_2010  = md("2010"),
             llegadas_2015 = md("2015"),
             llegadas_2020 = md("2020"),
             llegadas_2021 = md("Llegadas"),
             var_i_a_2021 = md("Var. i.a. (%)"),
             var_2019_2021 = md("Var. vs. 2019 (%)")) %>%
  tab_style(
    style = cell_text(weight =  "bold"),
...
El país del continente americano con mayor cantidad de ingresos económicos por turismo en `r as.character(anio_actual)` fue **`r data_tabla_america_ingresos %>% arrange(-ingresos_2021) %>% slice(1) %>% pull(pais)`**, con US\$ `r data_tabla_america_ingresos %>% arrange(-ingresos_2021) %>% slice(1) %>% pull(ingresos_2021) %>% round(1)` millones; en segundo lugar se ubicó **`r data_tabla_america_ingresos %>% arrange(-ingresos_2021) %>% slice(2) %>% pull(pais)`**, con US\$ `r data_tabla_america_ingresos %>% arrange(-ingresos_2021) %>% slice(2) %>% pull(ingresos_2021) %>% round(1)` millones; detrás de él estuvo **`r data_tabla_america_ingresos %>% arrange(-ingresos_2021) %>% slice(3) %>% pull(pais)`**, con US\$ `r data_tabla_america_ingresos %>% arrange(-ingresos_2021) %>% slice(3) %>% pull(ingresos_2021) %>% round(1)` millones; el cuarto lugar lo ocupó **`r data_tabla_america_ingresos %>% arrange(-ingresos_2021) %>% slice(4) %>% pull(pais)`**, con US\$ `r data_tabla_america_ingresos %>% arrange(-ingresos_2021) %>% slice(4) %>% pull(ingresos_2021) %>% round(1)` millones, la quinta posición fue para **`r data_tabla_america_ingresos %>% arrange(-ingresos_2021) %>% slice(5) %>% pull(pais)`**, con `r data_tabla_america_ingresos %>% arrange(-ingresos_2021) %>% slice(5) %>% pull(ingresos_2021) %>% round(1)` millones de dólares en `r as.character(anio_actual)`.

```{r ingresos-america-paises}
tabla_ingresos_america <- data_tabla_america_ingresos %>%
  select(pais,ingresos_2005,ingresos_2010,ingresos_2015,ingresos_2020,ingresos_2021,var_i_a_2021,var_2019_2021) %>% 
  mutate_at(vars(-pais),as.numeric) %>% 
  arrange(-ingresos_2021) %>% 
  slice(1:20) %>% 
  gt() %>% 
  # fmt_percent(columns=c(var_i_a_2021,var_2019_2021),decimals = 1,dec_mark = ",") %>% 
  gt_theme_dnmye() %>% 
  fmt_number( columns = c(ingresos_2005,ingresos_2010,ingresos_2015,ingresos_2020,ingresos_2021), decimals = 0, dec_mark = ",", sep_mark = ".") %>% 
  tab_caption("América: ingresos por turismo internacional. En millones de dólares. Primeros 20 países.") %>% 
  tab_spanner(columns = c(ingresos_2021,var_i_a_2021,var_2019_2021),
               label = md(glue("**{anio_actual}**"))) %>% 
  cols_label(pais = md(""),
             ingresos_2005 = md("2005"),
             ingresos_2010  = md("2010"),
             ingresos_2015 = md("2015"),
             ingresos_2020 = md("2020"),
             ingresos_2021 = md("Ingresos"),
             var_i_a_2021 = md("Var. i.a. (%)"),
             var_2019_2021 = md("Var. vs. 2019 (%)")) %>%
  tab_style(
    style = cell_text(weight =  "bold"),
