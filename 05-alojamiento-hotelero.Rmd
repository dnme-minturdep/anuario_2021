...
El objetivo del presente capítulo es **realizar una descripción cuantitativa y cualitativa del sector de alojamiento en la Argentina**. Con este fin, se brinda información en la primera sección de la oferta de alojamiento turístico colectivo para cada una de las provincias por tipo y categoría de los mismos a partir de la información provista por los organismos provinciales y municipales de turismo; mientras que en la segunda sección se analiza la dinámica oferta-demanda de la actividad hotelera y parahotelera a partir de la información de la Encuesta de Ocupación Hotelera (EOH).

## Oferta de Alojamiento Turístico Colectivo

La siguiente sección se elaboró con los datos extraídos del Padrón Único Nacional de Alojamiento (PUNA) que elabora anualmente la Dirección Nacional de Mercados y Estadística (DNMyE) con los datos recibidos por los organismos provinciales y municipales de turismo y el INDEC.
...
establecimientos_21 <- establecimientos_anual %>% slice(n()) %>% pull(establecimientos)
plazas_21 <- plazas_anual %>%   slice(n()) %>%   pull(plazas)
...
var_19_establecimientos <- (establecimientos_anual$establecimientos[dim(establecimientos_anual)[1]]/establecimientos_anual$establecimientos[dim(establecimientos_anual)[1]-2])-1

var_19_plazas <- (plazas_anual$plazas[dim(plazas_anual)[1]]/plazas_anual$plazas[dim(plazas_anual)[1]-2])-1
```

En el año `r anio_texto` se observaron un total de `r establecimientos_21 %>% lbl_int()` establecimientos (`r var_ia_establecimientos %>% lbl_percent()` i.a) y `r plazas_21 %>% lbl_int()` plazas (`r var_ia_plazas %>% lbl_percent()` i.a) . Respecto a 2019 (prepandemia) las variaciones fueron de `r var_19_establecimientos %>% lbl_percent()` y `r var_19_plazas %>% lbl_percent()`, respectivamente.
...
Durante el período 2015-`r anio_texto` las primeras tres categorías que tuvieron un mayor crecimiento de su oferta de establecimientos fueron: `r categorias_fraseo [1,2]` (`r categorias_1_porc %>% lbl_percent()`), `r categorias_fraseo [2,2]` (`r categorias_2_porc %>% lbl_percent()`) y `r categorias_fraseo [3,2]` (`r categorias_3_porc %>% lbl_percent()`). 
...
puna3
```
En las tablas a continuación se presenta la oferta de establecimientos y plazas según la clasificación del alojamiento (hoteleros, otros colectivos y parahoteleros) para los años 2015, 2020 y `r anio_texto`, como así también sus variaciones interanuales y punta a punta. 
...
tabla1 <- data_cuadro1 %>% 
  ...
  gt(groupname_col = "tipo") %>% 
   cols_label(
    ...
    var_ia_est_2021 = md("**Establecimientos**"),
    var_ia_pla_2021 = md("**Plazas**")
  ) %>% 
  ...
  cols_align(
    ...
    columns = c(establecimientos_2015, plazas_2015,
                   establecimientos_2021, plazas_2021,
                   var_ia_est_2021, var_ia_pla_2021))  %>% 
  tab_source_note(
    source_note = md(
  ...
  tab_spanner(
    ...
    columns = c(var_ia_est_2021, var_ia_pla_2021)) %>%
...
#gtsave(tabla1, "imagenes/tabla1_puna.png")
tabla1
```
...
data_cuadro2 <- rbind(total, data_cuadro2, totales) %>% 
  pivot_wider(names_from = "anio", 
              values_from = c("establecimientos", "plazas","var_ia_est","var_ia_pla")) %>% 
  select(-c(var_ia_est_2020, var_ia_pla_2020))
...
tabla2 <- data_cuadro2 %>% 
  ...
  gt(groupname_col = "tipo") %>% 
   cols_label(
    tipo = md(""),
    clasificacion_mintur = md("**Clasificación**"),
    establecimientos_2020 = md("**Establecimientos**"),
    plazas_2020 = md("**Plazas**"),
    establecimientos_2021  = md("**Establecimientos**") ,
    plazas_2021  = md("**Plazas**"),
    var_ia_est_2021 = md("**Establecimientos**"),
    var_ia_pla_2021 = md("**Plazas**")
  ...
  cols_align(
    align = "center",
    columns = c(establecimientos_2020, plazas_2020,
                   establecimientos_2021, plazas_2021,
                   var_ia_est_2021))  %>% 
  tab_source_note(
    source_note = md(
...
#gtsave(tabla2, "imagenes/tabla2_puna.png")
...
En la tabla a continuación podrá verificarse el ranking del año `r anio_texto` de alojamientos colectivos de las primeras 20 localidades con mayor oferta de plazas y su respectiva participación en el mismo. A su vez, se agregó la comparación de posiciones con el año 2015.      
...
#gtsave(tabla3, "imagenes/tabla3_puna.png")
tabla3
```
...
puna21 <- herramientas::read_file_srv("/srv/DataDNMYE/puna/procesamiento/puna2021.xlsx")
...
Al analizar la presencia de establecimientos pertenecientes a cadenas hoteleras en Argentina en el año `r anio_texto` se encontró que las primeras tres regiones con mayor porcentaje de establecimientos de cadenas fueron: `r cadenas_prov1` (`r cadenas_1_porc %>% lbl_percent()`), `r cadenas_prov2` (`r cadenas_2_porc %>% lbl_percent()`) y `r cadenas_prov3` (`r cadenas_3_porc %>% lbl_percent()`).   
...
```
Por último, a continuación se presenta, para el año `r anio_texto`, el ranking 10 de las principales cadenas hoteleras nacionales e internacionales, según su cantidad de plazas. A su vez se anexa la comparación con el mismo ranking para el año anterior. 
...
ranking_cad20 <- herramientas::read_file_srv("/srv/DataDNMYE/puna/procesamiento/puna2020.xlsx") %>% 
  filter(cadena_hotelera != "no corresponde") %>% 
  group_by(origen_de_la_cadena, cadena_hotelera) %>% 
  summarise(plazas = sum(plazas)) %>% 
  ungroup() %>% 
  group_by(origen_de_la_cadena) %>% 
  mutate(rank20 = as.integer(rank(-plazas))) %>% 
  ungroup() %>% 
  select(-plazas)
...
ranking_cad <- puna21 %>% 
  ...
  ungroup() %>% 
  left_join(ranking_cad20, by = c("origen_de_la_cadena","cadena_hotelera")) %>% 
  mutate(cadena_hotelera = str_to_title(cadena_hotelera))


tabla_nac <- ranking_cad %>% 
  filter(origen_de_la_cadena == "nacional") %>% 
  select(rank20, rank21, cadena_hotelera, plazas) %>% 
  gt() %>% 
   cols_label(
    rank20 = md("**Posición 2020**"),
    rank21 = md(glue("**Posición {Anio}**")),
  ...
  cols_align(
    align = "center",
    columns = c(rank20, rank21, plazas))  %>% 
  opt_table_font(
    font = "Encode Sans"
  ) %>%
  tab_source_note(
...
#gtsave(tabla_nac, "imagenes/tablaNac.png")
tabla_nac
```
...
#gtsave(tabla_int, "imagenes/tablaInt.png")
tabla_int
```
...
eoh1 <- eoh %>%
  ...
  mutate(var_1=round((pern_totales/lag(pern_totales,1)-1),3),
         ...
         var_1b=round((pern_totales/lag(pern_totales,2)-1),3),
         var_2b=round((pern_res/lag(pern_res,2)-1),3),
         var_3b=round((pern_nores/lag(pern_nores,2)-1),3),
...
eoh2 <- eoh %>%
  ...
  mutate(var_1=round((viaj_totales/lag(viaj_totales,1)-1),3),
         ...
         var_1b=round((viaj_totales/lag(viaj_totales,2)-1),3),
         var_2b=round((viaj_res/lag(viaj_res,2)-1),3),
         var_3b=round((viaj_nores/lag(viaj_nores,2)-1),3),
...
# eoh3 Pernoctaciones y viajeros hospedados en establecimientos
...
Las **estadías promedio** (expresadas en noches) ascendieron a `r format(round(1*eoh4[nrow(eoh4)-2,3],2),decimal.mark = ",",big.mark = ".")` (por debajo de la estadía observada en 2020, pero siendo superior a la estadía media de 2019). Cuando se diferencia por residencia, los residentes presentan estadías más elevadas que los no residentes: `r format(round(1*eoh4[nrow(eoh4)-1,3],2),decimal.mark = ",",big.mark = ".")` vs `r format(round(1*eoh4[nrow(eoh4),3],2),decimal.mark = ",",big.mark = ".")`.
...
ggplot(data = eoh4 %>% mutate(anio=factor(anio,levels = c(anio_ref-2,anio_ref-1,anio_ref)),
  ...
  facet_wrap(~anio)+
  geom_col()+
  #geom_text(position = position_stack(vjust = 1.02),size=2.5)+
...
Cuando se analiza la estacionalidad de la información presentada, se evidencia que en `r anio_texto` el mes con mayor cantidad de pernoctaciones fue **`r ranking_total[nrow(ranking_total),1]`**. Mismo mes se presenta primero en el ranking de pernoctaciones de residentes y no residentes.
...
En el caso de los **viajeros hospedados**, también se observaron los mayores registros en **`r ranking_total[nrow(ranking_total),1]`**.
...
Al diferenciar las estadías según residencia, se observa que en los **residentes** la región **`r ranking_res[nrow(ranking_res),1]`** presenta las estadías más elevadas (`r format(round(1*ranking_res[nrow(ranking_res),2],1),decimal.mark = ",",big.mark = ".")` noches), le sigue **`r ranking_res[nrow(ranking_res)-1,1]`** (`r format(round(1*ranking_res[nrow(ranking_res)-1,2],1),decimal.mark = ",",big.mark = ".")` noches) y **`r ranking_res[nrow(ranking_res)-2,1]`** (`r format(round(1*ranking_res[nrow(ranking_res)-2,2],1),decimal.mark = ",",big.mark = ".")` noches).

En el caso de los **no residentes**, las estadías más altas se presentaron en **`r ranking_nores[nrow(ranking_nores),1]`** (`r format(round(1*ranking_nores[nrow(ranking_nores),2],1),decimal.mark = ",",big.mark = ".")` noches), seguida por **`r ranking_nores[nrow(ranking_nores)-1,1]`** (`r format(round(1*ranking_nores[nrow(ranking_nores)-1,2],1),decimal.mark = ",",big.mark = ".")` noches) y **`r ranking_nores[nrow(ranking_nores)-2,1]`** (`r format(round(1*ranking_nores[nrow(ranking_nores)-2,2],1),decimal.mark = ",",big.mark = ".")` noches).
...
En **CABA**, **`r ranking_caba_res[nrow(ranking_caba_res),3]`** fue el mes con mayor cantidad de pernoctaciones de residentes y no residentes.
...
En **Córdoba**, **`r ranking_cor_res[nrow(ranking_cor_res),3]`** presentó la mayor cantidad de pernoctaciones de residentes.
...
Fue **`r ranking_cuy_res[nrow(ranking_cuy_res),3]`** el mes con más cantidad de pernoctaciones de residentes en la **Región Cuyo"**.
...
En la región **Litoral**, el mayor valor de pernoctes residentes se registró en **`r ranking_lit_res[nrow(ranking_lit_res),3]`**.
...
Pasando a la región **Norte**, **`r ranking_nor_res[nrow(ranking_nor_res),3]`** fue el mes que lideró el ranking de pernoctes de residentes.
...
Finalmente, en **Patagonia**, **`r ranking_pat_res[nrow(ranking_pat_res),3]`** presentó el mayor registro de pernoctes residentes y también de no residentes.
...
En **CABA**, **`r ranking_caba_res[nrow(ranking_caba_res),3]`** evidenció el mayor registro de viajeros residentes y no residentes.
...
En **Córdoba**, **`r ranking_cor_res[nrow(ranking_cor_res),3]`** lideró el ranking de viajeros residentes.
...
El mes que se destacó en la **Región Cuyo** fue **`r ranking_cuy_res[nrow(ranking_cuy_res),3]`**, evidenciando el mayor registro de viajeros residentes.
...
En **Litoral**, el mayor volumen de viajeros residentes se registró en **`r ranking_lit_res[nrow(ranking_lit_res),3]`**.
...
En la región **Norte**, **`r ranking_nor_res[nrow(ranking_nor_res),3]`** fue el mes con el registro más elevado de viajeros residentes.
...
Por último, en **Patagonia**, el mes con mayor cantidad de viajeros residentes y no residentes fue **`r ranking_pat_res[nrow(ranking_pat_res),3]`**.
...
# select(region_de_destino,total_hoteleros,var_1,`4_5_est`,var_2,`3_est`,
#        var_3,`1_2_est`,var_4,para_hoteles,var_5) 
...
Sin diferenciar por región de destino, en `r anio_texto` fueron los establecimientos **`r ranking_pern[nrow(ranking_pern),2]`** los que registraron mayor cantidad de pernoctaciones y viajeros. 
...
eoh18 <- eoh %>%
  ...
  mutate(var_1=round((establecimientos/lag(establecimientos,1)-1),3),
         ...
         var_1b=round((establecimientos/lag(establecimientos,2)-1),3),
         var_2b=round((plazas_disp/lag(plazas_disp,2)-1),3),
         var_3b=(top-lag(top,2))*100) %>%
  mutate(var_1b=case_when(anio==2021~var_1b,
                            TRUE~NA_real_),
         var_2b=case_when(anio==2021~var_2b,
                            TRUE~NA_real_),
         var_3b=case_when(anio==2021~var_3b,
                            TRUE~NA_real_)) %>% 
...
eoh22 <- eoh %>%
  ...
  mutate(var_1b=case_when(anio==2021~top-lag(top,2))) %>% 
  mutate(anio=as.character(anio))
...
eoh23 <- eoh %>%
  ...
  mutate(var_1b=case_when(anio==2021~top-lag(top,10))) %>% 
  filter(anio==2021)
...
eoh24 <- eoh %>%
  ...
  mutate(var_1b=case_when(anio==2021~top-lag(top,16))) %>% 
  filter(anio==2021)
