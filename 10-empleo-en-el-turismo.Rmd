...
base_eoh_region_2021 <- read_file_srv("/economia2/anuario/base_eoh_region_2021.csv")
anio_anuario <- "2021"
...
La primera sección procura cuantificar el volumen, la evolución y describir algunas características básicas de los empleos (personal ocupado o puestos de trabajo) en los hoteles y parahoteles para el período 2012-2021, según la información relevada por la Encuesta de Ocupación Hotelera (EOH).

En la segunda y última sección se presenta información de puestos de trabajo registrados en ramas características del turismo, la participación por género y la cantidad de empleadores para el período 2012-2021 a partir de la información publicada por el Centro de Estudios para la Producción (CEP XXI).
...
var_19_puestos <- ((tabla_datos$puestos_promedio_cantidad[dim(tabla_datos)[1]]/tabla_datos$puestos_promedio_cantidad[dim(tabla_datos)[1]-2])-1)*100
...
grafico_2_2_v2 <- grafico_2_2_v2_datos %>% 
  ...
  geom_text(data=filter(grafico_2_2_v2_datos, anio %in% c(2021,2019,2017,2015,2013)),
...
data_grafico_region_2021 <- base_eoh_region_2021 %>% 
  group_by(region) %>% 
  mutate(max=max(prom_puestos),
            min=min(prom_puestos)) %>% 
  filter(prom_puestos==max | prom_puestos==min) %>% 
...
grafico_2_5 <- ggplot()+
  geom_errorbar(data=data_grafico_region_2021, 
                aes(x=region, ymin=min, ymax=max), 
                width=.2, color = dnmye_colores("gris oscuro")) +
  geom_point(data=filter(base_eoh_region, anio==2021), aes(x=region, y=prom_puestos, group=""), 
             color=dnmye_colores(5), size = 3)+
  geom_text(data=filter(base_eoh_region, anio==2021), aes (x=region, y=prom_puestos, label=format (round (prom_puestos,0), big.mark=".",decimal.mark=",")),
            color = dnmye_colores(5),
            size = 3,  
            fontface = "bold",
            hjust = -.5,
            )+
  geom_label(data = data_grafico_region_2021, aes(x=region, y=prom_puestos, label= nombre_mes, vjust= ifelse(prom_puestos==min, 1.4, -.3)),
...
base_grafico_2_6_region <- base_eoh_region_2021 %>% 
  group_by(anio,region) %>% 
  mutate(coef_estac_PHT= sqrt(var(prom_puestos))/mean(prom_puestos),
...
entre el desvío estándar y el promedio de la cantidades mensuales, la estacionalidad de los PTH y de los pernoctes, lo que permite constatar, por un lado, que en todos los casos la variabilidad estacional de los pernoctes es mayor a la variabilidad en el volumen del personal ocupado y, por otro lado, que a mayor estacionalidad de los pernoctes mayor es la probabilidad de encontrar variabilidad en el volumen de PTH. No obstante, aunque se trata de una tendencia clara, esta relación no es lineal: por ejemplo, CABA presenta una variabilidad estacional de sus pernoctes algo mayor a la región de Córdoba (que sólo es superada por la Provincia de Buenos Aires), pero en esta última la estacionalidad de los PTH es más variable que en la CABA.
...
En esta última sección se analizan los puestos de trabajo registrados totales y por género, como así también las empresas correspondientes a las Ramas Características del Turismo (RCT) en el sector privado (formal) de la economía a partir de los datos abiertos publicados por el [Centro de Estudios para la Producción (CEP XXI)](https://datos.produccion.gob.ar/) en base al SIPA, para el período 2012-2021.
...
grafico_3_2 <-puestos_total_mes %>% 
  ...
  scale_y_continuous(limits = c(340000,400000), 
...
-   [Datos Abiertos:](https://datos.yvera.gob.ar/dataset?groups=sector-hotelero){target="_blank"} el portal de incluye un conjunto de datasets y recursos disponibles en diferentes niveles de desagregación y en variados formatos en caso de que se necesite un reprocesamiento de los mismos.
