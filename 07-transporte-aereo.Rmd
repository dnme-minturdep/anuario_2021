...
En el apartado siguiente, se estudian los vuelos de cabotaje considerando las frecuencias aéreas de los vuelos arribados mensualmente según las regiones, provincias y localidades de destino.
Adicionalmente, en el apartado que le continúa, se desarrolla la evolución del transporte aéreo internacional, considerando las frecuencias mensuales de los vuelos internacionales arribados a nuestro país, según diferentes variables de interés, tales como compañía aérea, aeropuertos de destino, etc.
...
cabotaje <- base_total %>% 
  ...
  filter(anio_local >= 2019 & anio_local <= 2021) %>% 
...
vuelos <- cabotaje %>% 
  ...
  pivot_wider(names_from = "anio_local", values_from = frecuencias) %>%
  mutate(var = round((anio_2021/anio_2020 - 1)*100, 2),
         var_19 = round((anio_2021/anio_2019 - 1)*100, 2)) 

vuelos_2019 <- vuelos[,1] %>% pull
vuelos_2020 <- vuelos[,2] %>% pull
vuelos_2021 <- vuelos[,3] %>% pull %>% format(., big.mark = ".")

var_19_factor <- if_else(vuelos$var_19 > 0,
                  "crecimiento del",
                  "decrecimiento del")

var_20_factor <- if_else(vuelos$var > 0,
                  "crecimiento del",
                  "decrecimiento del")

var_19 <- vuelos[,5] %>% pull %>% abs(.) %>% format(., decimal.mark = ",")

var_20 <- vuelos[,4] %>% pull %>% abs(.) %>% format(., decimal.mark = ",")

```

Al examinar la evolución de los vuelos de cabotaje realizados en el país, se verificó un total de `r vuelos_2021` frecuencias aéreas para el año `r as.character(anio)`, lo que implica un `r var_20_factor` `r var_20`% con respecto al año anterior y un `r var_19_factor` `r var_19`% comparado al 2019.
...
pasajeros <- cabotaje %>% 
  ...
  mutate(anio_local = paste0("anio_", anio_local)) %>% 
  pivot_wider(names_from = "anio_local", values_from = pasajeros) %>% 
  mutate(var = round((anio_2021/anio_2020 - 1)*100, 2),
         var_19 = round((anio_2021/anio_2019 - 1)*100, 2)) 

pasajeros_2019 <- pasajeros[,1] %>% pull
pasajeros_2020 <- pasajeros[,2] %>% pull
pasajeros_2021 <- pasajeros[,3] %>% pull %>% format(., big.mark = ".")

var_19_factor_pasajeros <- if_else(pasajeros$var_19 > 0,
                  "aumento del",
                  "decrecimiento del")

var_20_factor_pasajeros <- if_else(pasajeros$var > 0,
                  "incremento del",
                  "reducción del")

var_19_pasajeros <- pasajeros[,5] %>% pull %>% abs(.) %>% format(., decimal.mark = ",")

var_20_pasajeros <- pasajeros[,4] %>% pull %>% abs(.) %>% format(., decimal.mark = ",")
```

En el mismo año `r as.character(anio)`, `r pasajeros_2021` pasajeros viajaron en vuelos de cabotaje por el interior del país. Ello representa un `r var_20_factor_pasajeros` `r var_20_pasajeros`% con respecto al año anterior y un `r var_19_factor_pasajeros` `r var_19_pasajeros`% comparado al 2019.
...
asientos <- cabotaje %>% 
  ...
  mutate(anio_local = paste0("anio_", anio_local)) %>% 
  pivot_wider(names_from = "anio_local", values_from = asientos) %>% 
  mutate(var = round((anio_2021/anio_2020 - 1)*100, 2),
         var_19 = round((anio_2021/anio_2019 - 1)*100, 2))

asientos_2019 <- asientos[,1] %>% pull
asientos_2020 <- asientos[,2] %>% pull
asientos_2021 <- asientos[,3] %>% pull %>% format(., big.mark = ".")

var_19_factor_asientos <- if_else(asientos$var_19 > 0,
                  "incremento del",
                  "disminución del")

var_20_factor_asientos <- if_else(asientos$var > 0,
                  "crecimiento del",
                  "mengua del")

var_19_asientos <- asientos[,5] %>% pull %>% abs(.) %>% format(., decimal.mark = ",")

var_20_asientos <- asientos[,4] %>% pull %>% abs(.) %>% format(., decimal.mark = ",")
```

En total, `r asientos_2021` asientos fueron ofrecidos en el año `r as.character(anio)` en vuelos de cabotaje. Dicha cifra supone un `r var_20_factor_asientos` `r var_20_asientos`% con respecto al año anterior y una `r var_19_factor_asientos` `r var_19_asientos`% comparado al 2019.
...
#   filter(anio_local <= anio) %>% 
#   group_by(anio_local, mes_local) %>% 
#   summarise(vuelos = (sum(vuelos, na.rm = T) - val_refe$vuelos)/val_refe$vuelos,
#             pasajeros = (sum(pax_ad, na.rm = T) - val_refe$pasajeros)/val_refe$pasajeros,
#             asientos = (sum(asientos_pax, na.rm = T) - val_refe$asientos)/val_refe$asientos) %>%
#   mutate(fecha = lubridate::my(paste0(mes_local,"-", anio_local)),
#          across(.cols = c(vuelos, pasajeros, asientos), ~.*100)) %>%
#   ggplot() +
#   geom_line(aes(x = fecha,
#                 y = vuelos,
#                 color = "Vuelos"),
...
#   scale_x_date(date_breaks = "3 month", 
#                date_labels = "%b-%y")+
#   scale_color_manual(values = c("Vuelos" = dnmye_colores("amarillo"),
#                                 "Pasajeros" = dnmye_colores("cian"),
#                                 "Asientos" = dnmye_colores("rosa"))) +
#   labs(title = "Mercado de cabotaje",
#        subtitle = "Asientos, pasajeros y vuelos con respecto a enero 2017",
#        y = "Variación",
#        x = "",
#        color = "",
#        caption = "Fuente: MINTURDEP en base a información de ANAC") +
...
ranking20_2 <- ranking[2,1] %>% pull
ranking20_3 <- ranking[3,1] %>% pull
ranking20_4 <- ranking[4,1] %>% pull
ranking20_5 <- ranking[5,1] %>% pull
ranking20_6 <- ranking[6,1] %>% pull
ranking20_7 <- ranking[7,1] %>% pull

veces_primero_ultimo_20 <- round(max(ranking_20$frecuencias)/min(ranking_20$frecuencias), 1)


```

La **Ciudad de Buenos Aires** es la región del país a la que llega el mayor número de vuelos de cabotaje. A la misma le siguen **`r ranking_2`, `r ranking_3`, `r ranking_4`, `r ranking_5`, `r ranking_6` y `r ranking_7`**, en dicho orden. Comparativamente, la **Ciudad de Buenos Aires** recibe `r veces_primero_ultimo` veces más vuelos que la región que menos vuelos recibe (**`r ranking_7`**). En el año `r as.character(anio)` ese ratio fue de `r veces_primero_ultimo_20`, ubicándose nuevamente la región de **`r ranking20_7`** en último lugar.

<br>

```{r grafico-2, fig.cap= glue("Vuelos de cabotaje por región de destino. Años {anio-2} a {anio}.")}
cabotaje %>% 
  group_by(anio_local, region_destino) %>% 
  summarise(frecuencias = round(sum(vuelos, na.rm = T), 0)) %>%
  ggplot(aes(x = reorder(region_destino, -frecuencias), y = frecuencias)) +
  geom_bar(aes(fill = as.factor(anio_local)),
           position = position_dodge2(width = 0.9, padding = 0.1), stat = "identity") +
  geom_text(aes(label = format(frecuencias, big.mark = "."), group = as.factor(anio_local)),
...
vuelos_x_mes <- cabotaje %>% 
  ...
  mutate(mes_local = lubridate::month(dmy(paste("01", mes_local, "2020", sep = "-")),
                                      abbr = F, label = T))

max_2021 <- (vuelos_x_mes %>% 
  filter(`2021` == max(`2021`)))[1,4] %>% pull %>% format(., big.mark = ".")

mes_max_2021 <- (vuelos_x_mes %>% 
  filter(`2021` == max(`2021`)))[1,1] %>% pull 

min_2021 <- (vuelos_x_mes %>% mutate(across(.cols = c(2:4), ~ round(., 0))) %>% 
  filter(`2021` == min(`2021`)))[1,4] %>% pull %>% format(., big.mark = ".")

mes_min_2021 <- (vuelos_x_mes %>% mutate(across(.cols = c(2:4), ~ round(., 0))) %>% 
  filter(`2021` == min(`2021`)))[1,1] %>% pull  

max_2020 <- (vuelos_x_mes %>% mutate(across(.cols = c(2:4), ~ round(., 0))) %>% 
  filter(`2020` == max(`2020`)))[1,3] %>% pull %>% format(., big.mark = ".")

mes_max_2020 <- (vuelos_x_mes %>% mutate(across(.cols = c(2:4), ~ round(., 0))) %>% 
  filter(`2020` == max(`2020`)))[[1,1]]  

min_2020 <- (vuelos_x_mes %>% mutate(across(.cols = c(2:4), ~ round(., 0))) %>% 
  filter(`2020` == min(`2020`)))[[1,3]] 

mes_min_2020 <- (vuelos_x_mes %>% mutate(across(.cols = c(2:4), ~ round(., 0))) %>% 
  filter(`2020` == min(`2020`)))[[1,1]] 

```

Con respecto a la distribución mensual de los vuelos de cabotaje, se observa para el año en curso una mayor cantidad de vuelos aterrizados en el mes de `r mes_max_2021` con `r max_2021` frecuencias totales y la menor para el mes de `r mes_min_2021` con `r min_2021`.
Con respecto al año anterior, el máximo se encuentra en el mes de `r mes_max_2020` (`r max_2020`) y el mínimo en `r mes_min_2020` (`r min_2020`).

```{r tabla-1}
frq_reg_dest_21 <- cabotaje %>% 
  filter(anio_local == 2021) %>% 
  ...
  rename("total_21" = Total) %>% 
  mutate(total_21 = round(total_21, 0))

frq_reg_dest_20 <- cabotaje %>% 
  filter(anio_local == 2020) %>% 
  group_by(mes_local, region_destino, .drop = F) %>% 
  summarise(frecuencias = sum(vuelos)) %>% 
  mutate(mes_local = lubridate::month(mes_local, label = T, abbr = F)) %>% 
  pivot_wider(names_from = region_destino, values_from = frecuencias)
...
frq_reg_dest_21 <- frq_reg_dest_21 %>% 
  left_join(frq_reg_dest_20, by = "mes_local") %>% 
  ...
  mutate(across(.cols = c(2:21), ~ replace_na(., 0)),
         ...
         var_inter_caba_19 = if_else(!is.finite(var_inter_caba_19),
                                  ...
                                  var_inter_caba_19)) %>% 
    ungroup() %>% 
  mutate(across(.cols = everything(.), ~ replace(., is.infinite(.), 0)),
         across(.cols = contains("_x"), ~round(., 0))) %>%
  select(-c(16:23)) %>% 
  select(-contains("_y")) 

col_order <- c(1, 9, 2, 10, 17, 3, 13, 20, 8, 12, 19, 7, 11, 18, 5, 16, 23, 4, 15, 22, 6, 14, 21)

frq_reg_dest_21 <- frq_reg_dest_21[, col_order]

tabla_1_aerocomercial <- frq_reg_dest_21 %>%
  mutate(mes_local = str_to_title(mes_local)) %>% 
  ungroup() %>% 
  gt() %>% 
  # tab_header(
  #   #title = md("__FRECUENCIAS AÉREAS DE VUELOS DE CABOTAJE POR REGIÓN DE DESTINO SEGÚN MES__"),
  #   subtitle = "2021") %>% 
  tab_source_note(
  ...
  fmt_percent(columns = c(4, 5, 7, 8, 10, 11, 13, 14, 16, 17, 19, 20, 22, 23), decimals = 1, dec_mark = ",", sep_mark = ".") %>% 
    tab_spanner(
    label = "CABA",
    columns = c(caba_x,
                var_inter_caba,
...
tab_style(
 ...
 tab_style(
  ...
  tab_caption(glue("Vuelos de cabotaje por región de destino según mes. Años {anio-2} a {anio}."))
...
region_max_21 <- freq_prov_text %>% 
  filter(anio_local == max(anio_local)) %>% 
  filter(pct == max(pct))

region_max_2021_label <- region_max_21[,2] %>% pull
region_max_vuelos_2021_label <- region_max_21[,5] %>% pull %>% format(., decimal.mark = ",")

region_min_21 <- freq_prov_text %>% 
  filter(anio_local == max(anio_local)) %>% 
  filter(pct == min(pct))

region_min_2021_label <- region_min_21[,2] %>% pull
region_min_vuelos_2021_label <- region_min_21[,5] %>% pull %>% format(., decimal.mark = ",")



```

Del análisis del número de vuelos de cabotaje en el año `r as.character(anio)`, según región de destino, se observa que **`r region_max_2021_label`** concentra el `r region_max_vuelos_2021_label`% de los arribos.
Por su parte, la región de menor afluencia fue la **Región `r region_min_2021_label`** con una cuota de mercado del `r region_min_vuelos_2021_label`%.

<br>

```{r grafico-3, fig.cap= glue("Vuelos de cabotaje por región de destino. Años {anio-2} a {anio}.")}
...
cabotaje %>% 
  ...
  geom_text(aes(label = paste0(format(pct, decimal.mark = ","), "%"), vjust = -.5),
            ...
            #label.padding = unit(0.1, 'lines'), label.size = .01
            ) +
...
freq_reg_prov_loc <- cabotaje %>% 
  ...
  group_by(mes_local, region_destino, destino_provincia_etiqueta, destino_localidad_etiqueta, .drop = F) %>% 
  summarise(frecuencias = round(sum(vuelos, na.rm = T), 0)) %>% 
  mutate(mes_local = lubridate::month(mes_local, label = T, abbr = F)) %>% 
  pivot_wider(names_from = mes_local, values_from = frecuencias) %>%
  ungroup() %>% 
  mutate(across(.cols = c(4:15), ~ replace_na(., 0))) %>%
...
freq_bs_as <- freq_reg_prov_loc[1,16] %>% pull %>% format(., big.mark = ".")

freq_caba_loc <- freq_reg_prov_loc[2,3] %>% pull
freq_caba <- freq_reg_prov_loc[2,16] %>% pull %>% format(., big.mark = ".")

freq_cordoba_loc <- freq_reg_prov_loc[3,3] %>% pull
freq_cordoba <- freq_reg_prov_loc[3,16] %>% pull %>% format(., big.mark = ".")

freq_cuyo_loc <- freq_reg_prov_loc[4,3] %>% pull
freq_cuyo <- freq_reg_prov_loc[4,16] %>% pull %>% format(., big.mark = ".")

freq_litoral_loc <- freq_reg_prov_loc[5,3] %>% pull
freq_litoral <- freq_reg_prov_loc[5,16] %>% pull %>% format(., big.mark = ".")

freq_norte_loc <- freq_reg_prov_loc[6,3] %>% pull
freq_norte <- freq_reg_prov_loc[6,16] %>% pull %>% format(., big.mark = ".")

freq_patagonia_loc <- freq_reg_prov_loc[7,3] %>% pull
freq_patagonia <- freq_reg_prov_loc[7,16] %>% pull %>% format(., big.mark = ".")
```
...
freq_ciudad_dest_21 <- cabotaje %>% 
  ...
  summarise(frecuencias_21 = sum(vuelos)) %>% 
  ungroup() %>% 
  arrange(desc(frecuencias_21)) %>% 
  mutate(posicion_21 = 1:n(),
         participacion_21 = round(frecuencias_21/sum(frecuencias_21), 2))

freq_ciudad_dest_20 <- cabotaje %>% 
  filter(anio_local == 2020) %>% 
  group_by(destino_localidad_etiqueta) %>%   
  summarise(frecuencias_20 = sum(vuelos)) %>% 
  ungroup() %>% 
  arrange(desc(frecuencias_20)) %>% 
  mutate(posicion_20 = 1:n())

freq_ciudad_dest_19 <- cabotaje %>% 
  filter(anio_local == 2019) %>% 
  group_by(destino_localidad_etiqueta) %>%   
  summarise(frecuencias_19 = sum(vuelos)) %>% 
  ungroup() %>% 
  arrange(desc(frecuencias_19)) %>% 
  mutate(posicion_19= 1:n())

freq_ciudad_dest_21 <- freq_ciudad_dest_21 %>% 
  full_join(freq_ciudad_dest_20, by = "destino_localidad_etiqueta") %>% 
  full_join(freq_ciudad_dest_19, by = "destino_localidad_etiqueta") %>% 
  filter(!is.na(posicion_21) & !is.na(posicion_20) & !is.na(posicion_19)) %>% 
  mutate(variacion = round((frecuencias_21-frecuencias_20)/frecuencias_20, 2),
         variacion_19 = round((frecuencias_21-frecuencias_19)/frecuencias_19, 2),
         destino_localidad_etiqueta = case_when(posicion_21 >= 20 ~ "Resto",
                                                T ~ destino_localidad_etiqueta)) %>%
  group_by(destino_localidad_etiqueta) %>% 
  summarise(frecuencias_21 = sum(frecuencias_21),
            frecuencias_20 = sum(frecuencias_20),
            frecuencias_19 = sum(frecuencias_19),
            posicion_21 = first(posicion_21),
            posicion_20 = first(posicion_20),
            posicion_19 = first(posicion_19)) %>% 
  mutate(variacion = (frecuencias_21-frecuencias_20)/frecuencias_20,
         variacion_19 = (frecuencias_21-frecuencias_19)/frecuencias_19,
         participacion_21 = frecuencias_21/sum(frecuencias_21),
         posicion_21 = case_when(destino_localidad_etiqueta == "Resto" ~ "-",
                                             T ~ as.character(posicion_21)),
         posicion_20 = case_when(destino_localidad_etiqueta == "Resto" ~ "-",
                                             T ~ as.character(posicion_20)),
...
freq_ciudad_dest_21 <- freq_ciudad_dest_21[, col_order]

resto <- freq_ciudad_dest_21 %>% 
  filter(destino_localidad_etiqueta == "Resto")

freq_ciudad_dest_21 <- freq_ciudad_dest_21 %>% 
  filter(destino_localidad_etiqueta != "Resto") %>% 
  rbind(resto) %>% 
  mutate(across(.cols = contains("frecuencias_"), ~ round(., 0)))

primer_dest <- freq_ciudad_dest_21[1,1] %>% pull
segundo_dest <- freq_ciudad_dest_21[2,1] %>% pull
tercer_dest <- freq_ciudad_dest_21[3,1] %>% pull

freq_primer_dest <- freq_ciudad_dest_21[1,5] %>% pull %>%  format(., big.mark = ".")
freq_segundo_dest <- freq_ciudad_dest_21[2,5] %>% pull %>%  format(., big.mark = ".")
freq_tercer_dest <- freq_ciudad_dest_21[3,5] %>% pull %>%  format(., big.mark = ".")

top_dest <- freq_ciudad_dest_21 %>% 
  filter(destino_localidad_etiqueta != "Resto") %>% 
  top_n(3, variacion) %>% 
  arrange(-variacion) %>% 
...
not_top_dest <- freq_ciudad_dest_21 %>% 
  filter(destino_localidad_etiqueta != "Resto") %>% 
  top_n(-3, variacion) %>% 
  arrange(-variacion) %>% 
  select(destino_localidad_etiqueta, variacion) %>% 
...
Contrariamente, aquellos que menos crecieron (o decrecieron) fueron **`r not_top_primer_dest`** (`r not_top_freq_primer_dest`), **`r not_top_segundo_dest`** (`r not_top_freq_primer_dest`) y **`r not_top_tercer_dest` (`r not_top_freq_primer_dest`)**.

```{r tabla-3 }
tabla_3_aerocomercial <- freq_ciudad_dest_21 %>%
  gt() %>% 
  # tab_header(
 ...
 tab_style(
  ...
  tab_caption(glue("Ranking de ciudades de destino por vuelos de cabotaje. Años {anio-2} a {anio}."))
...
Al observar los vuelos de cabotaje según compañía aérea, **`r primer_aero`**, fue la que registró mayor cantidad de vuelos, explicando el `r freq_primer_aero` de las frecuencias aéreas de cabotaje del país (`r freq_abs_primer_aero`).
Le siguen **`r segundo_aero`** con `r freq_segundo_aero` (`r freq_abs_segundo_aero` frecuencias) y **`r tercer_aero`** con `r freq_tercer_aero` (`r freq_abs_tercer_aero`).

<br>

```{r grafico-4, fig.cap= glue("Vuelos de cabotaje, distribución porcentual por compañía aérea. Años {anio-2} a {anio}.")}
cabotaje %>% 
  mutate(empresa_agrup_def = if_else(! empresa_agrup_def %in% c("Aerolíneas Argentinas",
                                                                "Flybondi",
                                                                "JetSMART Airlines",
  ...
  geom_label(aes(y = pct+5, label = paste0(format(pct, decimal.mark = ","), "%")),
             size = 2.5) +
  facet_grid(~ anio_local) +
...
En relación a las ciudades de destino por compañía aérea, se observó que **`r primera_aero`** es la principal compañía aérea del país, conectando con `r primera_aero_destinos` localidades a lo largo de todo el territorio nacional.
En segundo lugar, le sigue **`r segunda_aero`**, con `r segunda_aero_destinos` destinos, y, en tercer lugar, **`r tercera_aero`** con `r tercera_aero_destinos`.
Por su parte, **LADE - Líneas Aéreas del Estado** ofrece vuelos hacia `r LADE_destinos` ciudades.
...
A continuación, se describe un gráfico de concentración para las distintas regiones en el año `r as.character(anio)`.
Esta visualización representa la participación acumulada de vuelos de cabotaje en cada región de destino; si todas las regiones recibiesen la misma cantidad de vuelos aterrizados, se observarían 7 compartimentos iguales (de alrededor de un 15% de los vuelos cada uno).
Como se percibe, en el año `r as.character(anio)`, la distribución de los vuelos de cabotaje según región de destino no fue equitativa dado que aproximadamente el 50% de los arribos se concentraron mayormente en la **CABA y Patagonia**, como sucedió en los años anteriores.

<br>

```{r grafico-5, fig.cap = glue("Proporción de vuelos de cabotaje por región de destino. Años {anio-2} a {anio}.")}
cabotaje %>% 
  group_by(anio_local, region_destino) %>% 
  summarise(frecuencias = sum(vuelos, na.rm = T)) %>% 
  group_by(anio_local) %>% 
  ...
  ungroup() %>% 
  mutate(pct = round((frecuencias/frecuencias_totales)*100, 1)) %>% 
  ggplot(aes(x = anio_local, y = pct, group = reorder(region_destino, pct))) +
...
herfintal_prov_20 <- cabotaje %>% 
  ...
  mutate(frecuencias_totales = sum(frecuencias, na.rm = T)) %>% 
  ungroup() %>% 
  mutate(cuota_20 = round((frecuencias/frecuencias_totales), 2),
         herfindal_20 = round(sum(cuota_20^2), 3)) %>% 
  select(destino_provincia_etiqueta, cuota_20, herfindal_20)
...
herfindal_prov1 <- data.table("destino_provincia_etiqueta" = "Índice de Herfindahl",
                              ...
                              "cuota_20" = herfintal_prov_20$herfindal_20[1],
                              "cuota_21" = herfintal_prov_21$herfindal_21[1]) 

herfindal_prov2 <- herfintal_prov_19 %>% 
  left_join(herfintal_prov_20, by = "destino_provincia_etiqueta") %>% 
...
IH_anio_pasado <- (herfindal_prov1 %>% 
                     mutate(across(.cols = c(2:4), ~ .*100)))[1, 3] %>% pull %>% format(., decimal.mark = ",")
IH_anio_base <- (herfindal_prov1 %>% 
                     mutate(across(.cols = c(2:4), ~ .*100)))[1, 2] %>% pull %>% format(., decimal.mark = ",")

IH_dif_vs_anio_anterior <- if_else(IH_anio_actual < IH_anio_pasado,
                                   paste("decrecimiento de",((herfindal_prov1 %>% 
                     mutate(across(.cols = c(2:4), ~ .*100)))[1, 4] %>% pull)-((herfindal_prov1 %>% 
                     mutate(across(.cols = c(2:4), ~ .*100)))[1, 3] %>% pull), "p.p."),
                                   paste("crecimiento de",((herfindal_prov1 %>% 
                     mutate(across(.cols = c(2:4), ~ .*100)))[1, 4] %>% pull)-((herfindal_prov1 %>% 
                     mutate(across(.cols = c(2:4), ~ .*100)))[1, 3] %>% pull), "p.p."))
...
IH_dif_vs_anio_base <- if_else(IH_anio_actual < IH_anio_base,
                     ...
                     mutate(across(.cols = c(2:4), ~ .*100)))[1, 4] %>% pull)-((herfindal_prov1 %>% 
                     mutate(across(.cols = c(2:4), ~ .*100)))[1, 2] %>% pull), 1), "p.p."),
                                   paste("crecimiento de",((herfindal_prov1 %>% 
                     mutate(across(.cols = c(2:4), ~ .*100)))[1, 4] %>% pull)-((herfindal_prov1 %>% 
                     mutate(across(.cols = c(2:4), ~ .*100)))[1, 2] %>% pull), "p.p."))
...
tabla_5_aerocomercial <- herfindal_prov2 %>% 
  ...
  tab_style(
    ...
    locations = cells_body(
      rows = destino_provincia_etiqueta == "Índice de Herfindahl")) %>% 
  tab_caption(glue("Índice de Herfindahl y participación porcentual de las vuelos de cabotaje por provincia de destino. Años {anio-2} a {anio}."))
...
herfintal_aerolinea_20 <- cabotaje %>% 
  filter(anio_local == 2020) %>%
  mutate(empresa_agrup_def = if_else(! empresa_agrup_def %in% c("Aerolíneas Argentinas",
                                                                "Flybondi",
                                                                "JetSMART Airlines",
  ...
  mutate(frecuencias_totales = sum(frecuencias, na.rm = T)) %>% 
  ungroup() %>% 
  mutate(cuota_20 = round((frecuencias/frecuencias_totales), 3),
         herfindal_20 = round(sum(cuota_20^2), 3)) %>% 
  select(empresa_agrup_def, cuota_20, herfindal_20)
...
herfintal_aerolinea<- data.table("empresa_agrup_def" = "Índice de Herfindahl",
                                 ...
                                 "cuota_20" = herfintal_aerolinea_20$herfindal_20[1],
                                 "cuota_21" = herfintal_aerolinea_21$herfindal_21[1])

herfintal_aerolinea2 <- herfintal_aerolinea_19 %>% 
  left_join(herfintal_aerolinea_20, by = "empresa_agrup_def") %>% 
...
En cuanto a la concentración de las frecuencias aéreas por compañía aérea, también se utilizó el IH.
En el año `r as.character(anio)` se registró un valor cercano a `r IH_aero_anio_actual`%, lo que implica un `r IH_aero_dif_vs_anio_anterior` respecto al año anterior, y un `r IH_aero_dif_vs_anio_base` con respecto al 2019.
En este marco, **Aerolíneas Argentinas** se posicionó como el principal actor del mercado, concentrando un `r IH_AA_anio_actual`% de las frecuencias aéreas totales de cabotaje al interior del país.
...
tabla_6_aerocomercial <- herfintal_aerolinea2 %>% 
  ...
  tab_style(
    ...
    locations = cells_body(
      rows = empresa_agrup_def == "Índice de Herfindahl")) %>% 
  tab_caption(glue("Índice de herfindahl y participación porcentual de vuelos de cabotaje por compañía aérea. Años {anio-2} a {anio}."))
...
vuelos_int_totales <- internacional %>% 
  ...
  summarise(vuelos = sum(vuelos, na.rm = T)) %>% 
  pivot_wider(names_from = anio_local, values_from = vuelos) %>%
  mutate(var_inter_anual = `2021`/`2020`*100-100,
         var_inter_2019 = `2021`/`2019`*100-10) 
...
vuelos_arribados_anterior_dif <- vuelos_int_totales[,4] %>% pull %>% abs() %>%round(1) %>% format(., big.mark = ".",decimal.mark=",") 
...
En el año `r as.character(anio)`, la conectividad aérea hacia la Argentina se estableció con `r vuelos_int_ciudades` ciudades en `r vuelos_int_paises` países, a través de vuelos directos a las mismas.
En dicho período, se registró un total de `r vuelos_arribados` vuelos internacionales que arribaron a Argentina, presentando `r condicion_vuelos_int` de `r vuelos_arribados_anterior_dif`%, en relación al año previo.

```{r grafico-6, fig.cap = glue("Vuelos internacionales totales desde y hacia Argentina. Años 2017 a {anio}.")}
# base_total %>%
#   filter(clasificacion_vuelo == "Internacional") %>%
#   filter(anio_local <= anio) %>%  
#   group_by(anio_local, mes_local) %>% 
...
int_asientos_2020 <- int_asientos[,2] %>% pull
int_asientos_2021 <- int_asientos[,3] %>% pull %>% format(., big.mark = ".")

int_var_19_factor_asientos <- if_else(int_asientos$var_19 > 0,
                  "aumento del",
                  "disminución del")

int_var_20_factor_asientos <- if_else(int_asientos$var > 0,
                  "crecimiento del",
                  "reducción del")

int_var_19_asientos <- int_asientos[,5] %>% pull %>% abs(.) %>% format(., decimal.mark = ",")

int_var_20_asientos <- int_asientos[,4] %>% pull %>% abs(.) %>% format(., decimal.mark = ",")
```

Contando el total de vuelos internacionales hacia y a Argentina en `r as.character(anio)`, hubo `r int_asientos_2021` plazas disponibles. Este valor supone una `r int_var_20_factor_asientos` `r int_var_20_asientos`% frente al año anterior y una `r int_var_19_factor_asientos` `r int_var_19_asientos`% con respecto al 2019.
...
int_pasajeros_2020 <- int_pasajeros[,2] %>% pull
int_pasajeros_2021 <- int_pasajeros[,3] %>% pull %>% format(., big.mark = ".")

int_var_19_factor_pasajeros <- if_else(int_pasajeros$var_19 > 0,
                  "incremento del",
                  "decrecimiento del")

int_var_20_factor_pasajeros <- if_else(int_pasajeros$var > 0,
                  "incremento del",
                  "merma del")

int_var_19_pasajeros <- int_pasajeros[,5] %>% pull %>% abs(.) %>% format(., decimal.mark = ",")

int_var_20_pasajeros <- int_pasajeros[,4] %>% pull %>% abs(.) %>% format(., decimal.mark = ",")
```

En total, `r int_pasajeros_2021` pasajeros viajaron en vuelos internacionales desde y hacia Argentina en todo el año. Al comparar ese valor contra un año atrás, esa cifra supone un `r var_20_factor_pasajeros` `r var_20_pasajeros`%. Sin embargo, cuando se toma ese valor frente dos períodos previos, el mismo supone un `r var_19_factor_pasajeros` `r var_19_pasajeros`%.
...
filter(anio_local == 2021) %>% 
  ...
  pivot_wider(names_from = origen_continente_etiqueta_indec, values_from = frecuencias_21)

freq_int_org_20 <- internacional %>% 
  filter(origen_pais_etiqueta != "Argentina") %>% 
  filter(anio_local == 2020) %>% 
  mutate(mes_local = str_to_title(lubridate::month(mes_local, label = T, abbr = F))) %>% 
  group_by(origen_continente_etiqueta_indec, mes_local, .drop = F) %>% 
  summarise(frecuencias_20 = sum(vuelos, na.rm = T)) %>%
  mutate(origen_continente_etiqueta_indec = paste0(origen_continente_etiqueta_indec, "_20")) %>% 
  pivot_wider(names_from = origen_continente_etiqueta_indec, values_from = frecuencias_20)
...
freq_int_org_19 <- internacional %>% 
  ...
  pivot_wider(names_from = origen_continente_etiqueta_indec, values_from = frecuencias_19) %>% 
  mutate(count_asia_19 = 0)

freq_int_org <- freq_int_org_21 %>% 
  left_join(freq_int_org_20, by = c("mes_local")) %>%
  left_join(freq_int_org_19, by = c("mes_local")) %>%
  janitor::clean_names() %>%
  mutate(across(.cols = -c(1), ~ replace_na(as.numeric(replace_na(., 0)))),
         var_inter_africa = round((africa_21/africa_20)-1, 1),
         var_inter_africa_19 = round((africa_21/count_africa_19)-1, 1),
         var_inter_asia = round((asia_21/asia_20)-1, 1),
         var_inter_asia_19 = round((asia_21/count_asia_19)-1, 1),
         var_inter_america = round((america_21/america_20)-1, 1),
         var_inter_america_19 = round((america_21/count_america_19)-1, 1),
         var_inter_europa = round((europa_21/europa_20)-1, 1),
         var_inter_europa_19 = round((europa_21/count_europa_19)-1, 1),
         var_inter_oceania = round((oceania_21/oceania_20)-1, 1),
         var_inter_oceania_19 = round((oceania_21/count_oceania_19)-1, 1),
...
tabla_7_aerocomercial <- freq_int_org %>%
  ...
  gt() %>% 
  # tab_header(
  #   title = md("__FRECUENCIAS AÉREAS DE VUELOS INTERNACIONALES A ARGENTINA POR CONTINENTE DE ORIGEN, POR MES__"),
  #   subtitle = "2021") %>% 
  tab_source_note(
    source_note = md("**Fuente:** DNMyE en base a información de ANAC.")) %>%
  cols_label(
...
internacional_x_continente_2021 <- internacional_x_continente %>% 
  select(anio_local, origen_continente_etiqueta_indec, pct) %>% 
  pivot_wider(names_from = origen_continente_etiqueta_indec,
              values_from = pct) %>% 
  filter(anio_local == 2021)

internacional_x_continente_2021_america <- internacional_x_continente_2021[,3] %>%  pull %>% format(., decimal.mark = ",")

internacional_x_continente_2021_europa <- internacional_x_continente_2021[,5] %>%  pull %>% format(., decimal.mark = ",")
```

Con respecto al origen de los vuelos internacionales según continente, es **América** la región con mayores frecuencias aéreas hacia Argentina (`r internacional_x_continente_2021_america`%), seguido por **Europa** (`r internacional_x_continente_2021_europa`%). Por otro lado, los continentes que registraron menor cantidad de vuelos internacionales hacia Argentina fueron **África, Asia y Oceanía** (menos del 1%).

<br>

```{r grafico-7, fig.cap = glue("Vuelos internacionales hacia Argentina por continente de origen. Años {anio-2} a {anio}.")}
...
primer_pais_orig_anio_prev <- (tabla_inter %>% arrange(-cuota_2020))[1,1] %>% pull()
segundo_pais_orig_anio_prev <- (tabla_inter %>% arrange(-cuota_2020))[2,1] %>% pull()
tercer_pais_orig_anio_prev <- (tabla_inter %>% arrange(-cuota_2020))[3,1] %>% pull()
```
...
Se observa que el orden de los países es similar al del año anterior.
...
freq_cont_pais_20 <- internacional %>%
  filter(origen_pais_etiqueta != "Argentina") %>% 
  filter(anio_local == 2020) %>%
  group_by(origen_continente_etiqueta_migraciones, origen_pais_etiqueta, empresa_agrup_def) %>% 
  summarise(frecuencias_20 = n())
...
freq_cont_pais <- freq_cont_pais_21 %>% 
  left_join(freq_cont_pais_20, by = c("origen_continente_etiqueta_migraciones", "origen_pais_etiqueta", "empresa_agrup_def")) %>% 
  ...
  mutate(across(.cols = -c(1:3), ~ as.numeric(.)),
         var_anual = round((frecuencias_21/frecuencias_20)-1, 3),
         var_anual_19 = round((frecuencias_21/frecuencias_19)-1, 3),
         # var_anual = if_else(!is.finite(var_anual),
         #                     frecuencias_21,
         #                     var_anual),
         # var_anual_19 = if_else(!is.finite(var_anual_19),
         #                     frecuencias_21,
         #                     var_anual_19),
         # frecuencias_20 = case_when(is.na(frecuencias_20) ~ "-",
         #                            T ~ as.character(frecuencias_20)),
         # frecuencias_19 = case_when(is.na(frecuencias_19) ~ "-",
         #                            T ~ as.character(frecuencias_19))
         ) %>% 
  group_by(origen_continente_etiqueta_migraciones, origen_pais_etiqueta, empresa_agrup_def) %>% 
...
tabla_8_aerocomercial <-  freq_cont_pais %>% 
 ...
 tab_style(
  ...
  tab_caption(glue("Vuelos internacionales por continente, país de origen y compañía aérea. Años {anio-2} a {anio}."))
...
vuelos_int_20 <- internacional %>% 
  filter(origen_pais_etiqueta != "Argentina") %>% 
filter(anio_local == 2020) %>%
  group_by(origen_localidad_etiqueta) %>% 
  summarise(frecuencias_20 = n()) %>% 
  arrange(-frecuencias_20) %>% 
  mutate(posicion_20 = 1:n()) 
...
vuelos_int <- vuelos_int_21 %>% 
  full_join(vuelos_int_20, by = "origen_localidad_etiqueta") %>% 
  full_join(vuelos_int_19, by = "origen_localidad_etiqueta") %>% 
  filter(!is.na(posicion_21) & !is.na(posicion_20) & !is.na(posicion_19)) %>% 
  mutate(variacion = round((frecuencias_21-frecuencias_20)/frecuencias_20, 2),
         variacion_19 = round((frecuencias_21-frecuencias_19)/frecuencias_19, 2),
         origen_localidad_etiqueta = case_when(posicion_21 >= 20 ~ "Resto",
                                                T ~ origen_localidad_etiqueta)) %>%
  group_by(origen_localidad_etiqueta) %>% 
  summarise(frecuencias_21 = sum(frecuencias_21),
            frecuencias_20 = sum(frecuencias_20),
  ...
  mutate(variacion = round((frecuencias_21-frecuencias_20)/frecuencias_20, 2),
         variacion_19 = round((frecuencias_21-frecuencias_19)/frecuencias_19, 2),
         participacion_21 = round(frecuencias_21/sum(frecuencias_21), 3),
         posicion_21 = case_when(origen_localidad_etiqueta == "Resto" ~ "-",
                                 T ~ as.character(posicion_21)),
         posicion_20 = case_when(origen_localidad_etiqueta == "Resto" ~ "-",
                                 T ~ as.character(posicion_20)),
...
Por lo contrario, **`r ciudad_int_rank_ult`, `r ciudad_int_rank_anteult` y `r ciudad_int_rank_antepenult`** fueron las ciudades que registraron las mayores caídas respecto al año anterior (`r ciudad_int_rank_ult_pct`%, `r ciudad_int_rank_anteult_pct`% y `r ciudad_int_rank_antepenult_pct`% respectivamente).

```{r tabla-9}
tabla_9_aerocomercial <- vuelos_int %>%
  gt() %>% 
  # tab_header(
  #   title = md("__RANKING DE FRECUENCIAS AÉREAS DE VUELOS DE CABOTAJE POR CIUDAD DE DESTINO__")) %>% _frq
  tab_source_note(
    source_note = md("**Fuente:** DNMyE en base a información de ANAC.")) %>% 
  cols_label(
 ...
 tab_style(
  ...
  tab_caption(glue("Vuelos internacionales hacia Argentina por ciudad de origen. Años {anio-2} a {anio}."))
...
ranking_int_partcipacion <- vuelos_int %>% 
  select(origen_localidad_etiqueta, posicion_21, posicion_20) %>% 
  mutate(puestos = as.numeric(posicion_20) - as.numeric(posicion_21)) %>% 
  arrange(-puestos)

rank_particip_primer <- ranking_int_partcipacion[1,1] %>% pull
rank_particip_segundo <- ranking_int_partcipacion[2,1] %>% pull

# rank_particip_primer_posiciones <- ranking_int_partcipacion[1,4] %>% pull
# rank_particip_segundo_posiciones <- ranking_int_partcipacion[2,4] %>% pull

rank_particip_primer_posicion_21 <- ranking_int_partcipacion[1,2] %>% pull
rank_particip_segundo_posiciones_21 <- ranking_int_partcipacion[2,2] %>% pull

rank_particip_primer_posicion_20 <- ranking_int_partcipacion[1,3] %>% pull
rank_particip_segundo_posiciones_20 <- ranking_int_partcipacion[2,3] %>% pull

```

Con respecto a las posiciones en el ranking `r as.character(anio)`, la ciudad que más escaló fue **`r rank_particip_primer`**, ubicándose en el puesto número N° `r rank_particip_primer_posicion_21` luego de haber estado el año anterior en la posición N° `r rank_particip_primer_posicion_20`.
A la misma, le sigue **`r rank_particip_segundo`**, la cual pasó del puesto `r rank_particip_segundo_posiciones_20` al `r rank_particip_segundo_posiciones_21`.
...
tabla_10_aerocomercial <- internacional %>% 
  ...
  gt() %>% 
  # tab_header(
  #   title = md("__FRECUENCIAS AÉREAS DE VUELOS INTERNACIONALES POR MES, SEGÚN PAÍS Y CIUDAD DE ORIGEN__"),
  #   subtitle = md("__2021__")) %>% 
  tab_source_note(
    source_note = md("**Fuente:** DNMyE en base a información de ANAC.")) %>%
  cols_label(
...
rank_aero_int_primer <- (int_ranking_aero %>% filter(empresa_agrup_def != "Otras Aerolíneas"))[1,1] %>% pull()
rank_aero_int_primer_frq <- (int_ranking_aero %>% filter(empresa_agrup_def != "Otras Aerolíneas"))[1,2] %>% pull() %>% format(., big.mark = ".") 
rank_aero_int_primer_pct <- (int_ranking_aero %>% filter(empresa_agrup_def != "Otras Aerolíneas"))[1,3] %>% pull() %>% format(., decimal.mark = ",") 

rank_aero_int_segundo <- (int_ranking_aero %>% filter(empresa_agrup_def != "Otras Aerolíneas"))[2,1] %>% pull()
rank_aero_int_segundo_frq <- (int_ranking_aero %>% filter(empresa_agrup_def != "Otras Aerolíneas"))[2,2] %>% pull() %>% format(., big.mark = ".") 
rank_aero_int_segundo_pct <- (int_ranking_aero %>% filter(empresa_agrup_def != "Otras Aerolíneas"))[2,3] %>% pull() %>% format(., decimal.mark = ",") 
```
...
int_ranking_aero %>% 
  ggplot(aes(x = factor(empresa_agrup_def, levels = c("Aerolíneas Argentinas",
             ...
             y = pct)) +
  geom_bar(aes(fill = empresa_agrup_def), stat = "identity", show.legend = F) +
  geom_label(aes(label = paste0(format(pct, decimal.mark = ","), "%"), y = pct +3),
             size = 2.5) +
  scale_y_continuous(limits = c(0, 50),
                     labels = function(x) paste0(x, "%")) +
  scale_x_discrete(labels = function(empresa_agrup_def) str_wrap(empresa_agrup_def, width = 10))+
...
aerolineas_int_20 <- internacional %>% 
  filter(origen_pais_etiqueta != "Argentina") %>% 
filter(anio_local == 2020) %>%
  mutate(empresa_agrup_def = case_when(is.na(empresa_agrup_def) ~ "Otras Aerolineas",
                                     T ~ as.character(empresa_agrup_def))) %>%
...
aerolineas_int <- aerolineas_int_21 %>% 
  full_join(aerolineas_int_20, by = "empresa_agrup_def") %>% 
  full_join(aerolineas_int_19, by = "empresa_agrup_def") %>% 
  filter(!is.na(posicion_21) & !is.na(posicion_20) & !is.na(posicion_19)) %>% 
  mutate(variacion = round((frecuencias_21 - frecuencias_20)/frecuencias_20, 2),
         variacion_19 = round((frecuencias_21 - frecuencias_19)/frecuencias_19, 2),
         empresa_agrup_def = case_when(posicion_21 >= 20 ~ "Resto",
                                                T ~ empresa_agrup_def)) %>%
  group_by(empresa_agrup_def) %>% 
  summarise(frecuencias_21 = sum(frecuencias_21),
            frecuencias_20 = sum(frecuencias_20),
  ...
  mutate(variacion = round((frecuencias_21 - frecuencias_20)/frecuencias_20, 2),
         ...
         posicion_21 = case_when(empresa_agrup_def == "Resto" ~ "-",
                                 T ~ as.character(posicion_21)),
         posicion_20 = case_when(empresa_agrup_def == "Resto" ~ "-",
                                 T ~ as.character(posicion_20)),
...
ranking_aero_partcipacion <- aerolineas_int %>% 
  select(empresa_agrup_def, posicion_21, posicion_20) %>% 
  mutate(puestos = as.numeric(posicion_20) - as.numeric(posicion_21)) %>% 
  arrange(-puestos) 
...
aero_rank_particip_primer_posiciones <- ranking_int_partcipacion[1,4] %>% pull
aero_rank_particip_segundo_posiciones <- ranking_int_partcipacion[2,4] %>% pull

aero_rank_particip_primer_posicion_21 <- ranking_aero_partcipacion[1,2] %>% pull
aero_rank_particip_segundo_posiciones_21 <- ranking_aero_partcipacion[2,2] %>% pull

aero_rank_particip_primer_posicion_20 <- ranking_aero_partcipacion[1,3] %>% pull
aero_rank_particip_segundo_posiciones_20 <- ranking_aero_partcipacion[2,3] %>% pull
```

Una de las compañías que ganó más posiciones en el año `r as.character(anio)` fue **`r aero_rank_particip_primer`**, subiendo `r aero_rank_particip_primer_posiciones` posiciones en relación al año anterior (del puesto N° `r aero_rank_particip_primer_posicion_20` al N° `r aero_rank_particip_primer_posicion_21`).
En segundo lugar, se encuentra **`r aero_rank_particip_segundo`**, pasando de la posición N° `r aero_rank_particip_segundo_posiciones_20` a la N° `r aero_rank_particip_segundo_posiciones_21`.
...
# internacional %>% 
#   filter(tipo_movimiento == "Aterrizaje" & anio_local == 2020) %>%
...
herfintal_int_20 <- internacional %>%  # OJO CON ESTE CASO
  filter(anio_local == 2020) %>% 
      filter(origen_pais_etiqueta != "Argentina") %>% 
group_by(anio_local, origen_pais_etiqueta) %>% 
  summarise(frecuencias = sum(vuelos, na.rm = T)) %>% 
  ...
  mutate(cuota_20 = round((frecuencias/frecuencias_totales), 3),
         herfindal_20 = round(sum(cuota_20^2), 3)) %>% 
  select(origen_pais_etiqueta, cuota_20, herfindal_20)
...
herfintal_int_pais <- data.table("origen_pais_etiqueta" = "Índice de Herfindahl",
                                 ...
                                 "cuota_20" = herfintal_int_20$herfindal_20[1],
                                 "cuota_21" = herfintal_int_21$herfindal_21[1])

herfintal_int_pais2 <- herfintal_int_19 %>% 
  left_join(herfintal_int_20, by = "origen_pais_etiqueta") %>% 
...
tabla_13_aerocomercial <- herfintal_int_pais2 %>% 
  ...
  tab_caption(glue("Índice de Herfindahl y participación porcentual de vuelos internacionales por país de origen. Años {anio-2} a {anio}."))
...
herfintal_aero_int_20 <- internacional %>% 
    filter(origen_pais_etiqueta != "Argentina") %>% 
filter(anio_local == 2020) %>%
  group_by(anio_local, empresa_agrup_def) %>% 
  summarise(frecuencias = sum(vuelos, na.rm = T)) %>% 
...
herfintal_aerolinea_int<- data.table("empresa_agrup_def" = "Índice de Herfindahl",
                                 ...
                                 "cuota_20" = herfintal_aero_int_20$herfindal_20[1],
                                 "cuota_21" = herfintal_aero_int_21$herfindal_21[1])

herfintal_aerolinea_int2 <- herfintal_aero_int_19 %>% 
  left_join(herfintal_aero_int_20, by = "empresa_agrup_def") %>% 
...
aeros_int_20 <- n_distinct(herfintal_aero_int_20$empresa_agrup_def)

```

En el año `r as.character(anio)`, volaban al país `r aeros_int_21` aerolíneas, en comparación a las `r aeros_int_20` que lo hacían un año antes.
Para medir el nivel de concentración de vuelos internacionales según compañía aérea, se utilizó aquí también el Índice de Herfindahl.
Para el `r as.character(anio)`, el índice registra un valor de `r herfindah_aero_int`%, lo que supone `r herfindah_aero_int_cond`p.p.
con respecto al año anterior.
En este contexto, **Aerolíneas Argentinas** fue la empresa que registró la mayor cantidad de vuelos internacionales, aproximadamente un tercio del total de frecuencias, seguida por **LATAM**.
...
tabla_14_aerocomercial <- herfintal_aerolinea_int2 %>% 
  ...
  tab_style(
    ...
    locations = cells_body(
      rows = empresa_agrup_def == "Índice de Herfindahl")) %>% 
  tab_caption(glue("Índice de Herfindahl y participación porcentual de vuelos internacionales por compañía aérea. Años {anio-2} a {anio}."))
