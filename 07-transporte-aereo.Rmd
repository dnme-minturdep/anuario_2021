...
Adicionalmente, en la sección que le continúa, se desarrolla la evolución del transporte aéreo internacional, considerando las frecuencias mensuales de los vuelos internacionales arribados a nuestro país, según diferentes variables de interés, tales como compañía aérea, aeropuertos de destino, etc.
...
cabotaje <- base_total %>% 
  ...
  filter(anio_local >= anio_base & anio_local <= anio) %>% 
...
herfintal_int_20 <- internacional %>% 
  filter(anio_local == (anio - 1)) %>% 
      filter(origen_pais_etiqueta != "Argentina") %>% 
group_by(anio_local, origen_pais_etiqueta) %>% 
  summarise(frecuencias = sum(vuelos, na.rm = T)) %>% 
